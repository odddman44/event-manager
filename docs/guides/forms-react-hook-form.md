...
> ⚠️ **현재 상태**: 이 프로젝트에는 `react-hook-form`, `@hookform/resolvers`, `zod`가 아직 설치되어 있지 않습니다. 실제 인증 폼(`login-form.tsx`, `sign-up-form.tsx` 등)은 `useState` + Supabase 클라이언트 SDK(`supabase.auth.signInWithPassword` 등) 직접 호출 패턴을 사용합니다. 이 문서는 폼에 스키마 검증/복잡한 상태관리가 필요해질 때 도입할 패턴 가이드입니다.
...
- [ ] `npm run format:check` 통과 (또는 커밋 시 lint-staged가 자동 처리)
- [ ] 불필요한 리렌더링 없음
