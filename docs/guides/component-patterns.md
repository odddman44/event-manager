...
이 문서는 Next.js 16 + React 19(설치 버전 19.2.7) 환경에서 효율적이고 재사용 가능한 컴포넌트 작성 패턴을 제공합니다.
...
> ⚠️ `react-window`는 이 프로젝트에 설치되어 있지 않습니다. 큰 리스트 렌더링이 실제로 필요해지면 `npm install react-window`로 추가한 뒤 아래 패턴을 적용하세요.

```tsx
// ✅ 큰 리스트 가상화 (react-window 설치 후 사용)
