...
이 문서는 TailwindCSS v3(설치 버전 3.4.19) + shadcn/ui를 활용한 스타일링 규칙과 모범 사례를 제공합니다.

## 🎨 기술 스택 개요

### 핵심 스타일링 도구 (실제 설치된 패키지 기준)

- **TailwindCSS v3.4.19**: 유틸리티 기반 CSS 프레임워크 (`tailwind.config.ts` 기반 클래식 설정)
- **shadcn/ui**: Radix UI 기반 컴포넌트 라이브러리 (`new-york` style, baseColor `neutral`)
- **next-themes**: 다크모드 지원
- **tailwindcss-animate**: 애니메이션 플러그인 (`tailwind.config.ts`의 `plugins`에 등록됨. v4용 `tw-animate-css`가 아님)
- **CSS Variables**: 동적 테마 시스템 (`app/globals.css`)
- **class-variance-authority + tailwind-merge**: `lib/utils.ts`의 `cn()` 헬퍼가 내부적으로 사용

> ✅ Prettier + `prettier-plugin-tailwindcss`가 설치되어 있어 `npm run format`(또는 커밋 시 lint-staged)이 className 순서를 자동으로 정렬합니다. 아래 순서 가이드는 자동 정렬이 적용되는 원리를 이해하기 위한 참고용입니다.
...
`prettier-plugin-tailwindcss`가 자동으로 정렬해주지만, 직접 작성할 때는 다음 순서를 참고하세요:
...
`app/globals.css`에 정의된 색상 변수 (shadcn/ui `baseColor: "neutral"` 기준, 실제 값):
...
`tailwind.config.ts`에서 `import tailwindcssAnimate from "tailwindcss-animate"` 후 `plugins: [tailwindcssAnimate]`로 등록되어 있습니다(ESLint `@typescript-eslint/no-require-imports` 규칙 때문에 `require()` 대신 정적 import 사용). 컴포넌트 코드에서는 별도 import 없이 바로 유틸리티 클래스를 사용할 수 있으며, Radix UI 기반 컴포넌트(dropdown-menu 등)의 열림/닫힘 트랜지션에 이미 쓰이고 있습니다.

```tsx
// ✅ 실제 components/ui/dropdown-menu.tsx에서 사용 중인 패턴
